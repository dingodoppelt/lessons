...
//$schueler = $db->query("SELECT * FROM schueler WHERE ende > NOW() || ende IS NULL && gruppe IS NULL;") or die($db->error);
...
if (isset($_GET['schuelerWahl'])) {
    ...
    //$teilnehmerX = $db->query("SELECT ID, vorname, nachname FROM schueler WHERE vertraege.gruppe = " .$_GET['schuelerWahl'] . ";") or die($db->error);
